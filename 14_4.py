...
def plot_color_palette(colors, main_topic):
    # Bokeh 바 차트 생성
    p = figure(x_range=colors, height=350, title=f"Recommended Palette for {main_topic}",
               toolbar_location=None, tools="", y_axis_label="Color Intensity")
    p.vbar(x=colors, top=[1] * len(colors), width=0.9,
...
for sentence in sentences:
    ...
    plot_color_palette(recommended_palette, main_topic)
    print("\n")
